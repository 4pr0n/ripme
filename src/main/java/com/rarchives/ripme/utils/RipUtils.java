...
public class RipUtils {
    ...
    private static final String M_IMGUR_COM = "m." + IMGUR_COM;
    private static final String IMGUR_ALBUM_START_URL = IMGUR_COM + "/a/";
    ...
    public static List<URL> getFilesFromURL(URL url) {
        ...
        // Imgur album
        if ((url.getHost().endsWith(IMGUR_COM)) && url.toExternalForm().contains(IMGUR_ALBUM_START_URL)) {
            try {
        ...
        if (url.getHost().equals(IMGUR_COM) || M_IMGUR_COM.equals(url.getHost())) {
            try {
                // Fetch the page
    ...
    private static String urlFromImgurDirectoryName(String dir) {
        ...
        if (containsNewOrTop(fields) && (containsWeekMonthYear(fields) || fields.contains("all"))) {
            ...
            // Series of images
            url += IMGUR_COM + "/" + album.replaceAll("-", ",");
        } else if (album.length() == 5 || album.length() == 6) {
            // Album
            url += IMGUR_ALBUM_START_URL + album;
        } else {
            // User account
            url += album + "." + IMGUR_COM + "/";
            if (fields.size() > 2) {
