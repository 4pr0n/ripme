...
public class PhotobucketRipper extends AlbumRipper {
    ...
    public String getGID(URL url) throws MalformedURLException {
        ...
        Pattern p = Pattern.compile("^https?://[a-zA-Z0-9]+\\.photobucket\\.com/user/([a-zA-Z0-9_\\-]+)/library.*$");
        ...
        throw new MalformedURLException(
                "Expected photobucket.com gallery formats: http://x###.photobucket.com/username/library/... Got: " + url
        );
    }
    ...
    public void rip() throws IOException {
        ...
        List<String> rippedSubs = new ArrayList<>();
    ...
    public List<String> ripAlbumAndGetSubalbums(String theUrl) throws IOException {
        ...
        while (pageIndex == 0 || filesIndex < filesTotal) {
            ...
            for (Element script : albumDoc.select("script[type=text/javascript]")) {
                ...
                // Grab the JSON
                Pattern p = Pattern.compile("^.*collectionData: (\\{.*\\}).*$", Pattern.DOTALL);
            ...
            for (int i = 0; i < objects.length(); i++) {
                ...
                filesIndex += 1;
                addURLToDownload(new URL(image), "", object.getString("location").replaceAll(" ", "_"),
                        albumDoc.location(), pageResponse.cookies());
            }
        }
        // Get subalbums
        if (url != null)
    ...
    private List<String> getSubAlbums(String url, String currentAlbumPath) {
        ...
        try {
            ...
            for (int i = 0; i < subalbums.length(); i++) {
                String suburl = "http://" + subdomain + ".photobucket.com" + subalbums.getJSONObject(i).getString("path");
