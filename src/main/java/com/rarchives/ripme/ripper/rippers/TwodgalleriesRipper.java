...
public class TwodgalleriesRipper extends AbstractHTMLRipper {
    ...
    public String getGID(URL url) throws MalformedURLException {
        Pattern p = Pattern.compile("^.*2dgalleries.com/artist/([a-zA-Z0-9\\-]+).*$");
    ...
    private String getURL(String userid, int offset) {
        return "http://en.2dgalleries.com/artist/" + userid + "?timespan=4"
                + "&order=1" + "&catid=2" + "&offset=" + offset + "&ajx=1&pager=1";
    }
    ...
    public Document getNextPage(Document doc) throws IOException {
        ...
        String url = getURL(getGID(this.url), offset);
        sleep(500);
        Document nextDoc = Http.url(url).cookies(cookies).get();
