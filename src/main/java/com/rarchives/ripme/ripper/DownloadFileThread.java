...
public class DownloadFileThread extends Thread {
    ...
    private static final String WHILE_DOWNLOADING = " while downloading ";
        ...
        do {
            ...
            try {
                ...
                if (statusCode / 100 == 4) { // 4xx errors
                    LOGGER.error("[!] Non-retriable status code " + statusCode + WHILE_DOWNLOADING + "from " + url);
                    observer.downloadErrored(url, "Non-retriable status code " + statusCode + WHILE_DOWNLOADING + url.toExternalForm());
                    return; // Not retriable, drop out.
                }

                if (statusCode / 100 == 5) { // 5xx errors
                    observer.downloadErrored(url, "Retriable status code " + statusCode + WHILE_DOWNLOADING + url.toExternalForm());
                    // Throw exception so download can be retried
                ...
                LOGGER.error("[!] HTTP status " + hse.getStatusCode() + WHILE_DOWNLOADING + "from " + urlToDownload);
                if (hse.getStatusCode() == 404 && Utils.getConfigBoolean("errors.skip404", false)) {
                    observer.downloadErrored(url, "HTTP status code " + hse.getStatusCode() + WHILE_DOWNLOADING + url.toExternalForm());
                    return;
                }
            } catch (IOException e) {
                LOGGER.debug("IOException", e);
                LOGGER.error("[!] Exception" + WHILE_DOWNLOADING + "file: " + url + " - " + e.getMessage());
            } finally {
