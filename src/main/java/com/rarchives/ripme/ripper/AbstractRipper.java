...
public abstract class AbstractRipper extends Observable implements RipperInterface, Runnable {
    ...
    public boolean addURLToDownload(URL url, String prefix, String subdirectory, String referrer, Map<String, String> cookies) {
        try {
            ...
            LOGGER.debug("Ripper has been stopped", e);
            return false;
        }
        ...
        try {
            ...
            saveFileAs = new File(workingDir.getCanonicalPath() + subdirectory + File.separator + prefix + saveAs);
        } catch (IOException e) {
     ...
     *
     * @param url    Url from file
     * @param reason Reason for not download file
     */
    ...
    public static AbstractRipper getRipper(URL url) throws Exception {
        for (Constructor<?> constructor : getRipperConstructors("com.rarchives.ripme.ripper.rippers")) {
            try {
                ...
                LOGGER.info("Incompatible ripper: ", e);
            }
        }
     ...
     *
     * @param status - status from update
